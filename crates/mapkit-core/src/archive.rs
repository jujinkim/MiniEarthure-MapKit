...
/// Source/schema/dependency fingerprint invalidates disposable generated data.
pub fn archive_key(world: &str, cell: Cell) -> String {
    sha256(
        format!(
            "MKCELL01:{BUILD_FINGERPRINT}:{GENERATED_VERSION}:{world}:{}:{}",
            cell.x, cell.y
        )
        .as_bytes(),
    )
...
fn invalid() -> Error {
    error(
        "E_ARCHIVE",
        "invalid, incompatible or damaged generated-cell archive",
    )
}
...
pub fn encode_archive(chunk: &GeneratedChunk, key: &str, max_bytes: u64) -> Result<Vec<u8>> {
    let mut convex = vec![];
    {
        ...
        crate::cancellation::checkpoint()?;
            ...
            for v in c.shape.vertices.iter().flatten() {
                convex.extend_from_slice(&v.to_le_bytes());
            }
    ...
    let size = convex.len()
        ...
        + chunk
            .building_prisms
            .iter()
            .map(|p| 92 + p.object_id.len() + p.material.len() + p.usage.len())
            .sum::<usize>();
    if size as u64 > max_bytes {
        return Err(error("E_BUDGET", "cell archive exceeds byte allowance"));
    }
    if key.len() != 64 || chunk.format_version != GENERATED_VERSION {
        return Err(invalid());
    }
...
pub fn decode_archive(
    ...
    if bytes.len() < HEADER || bytes.len() as u64 > archive_limit(cost) {
        return Err(invalid());
    }
    let mut r = Reader { bytes, offset: 0 };
    let magic = r.take(8)?;
    if magic != MAGIC || r.take(64)? != key.as_bytes() {
        return Err(invalid());
    }
    let hash = r.take(64)?;
    if r.u32()? != GENERATED_VERSION || r.u32()? as i32 != cell.x || r.u32()? as i32 != cell.y {
        return Err(invalid());
    }
    ...
    for _ in 0..triangles {
        ...
        let spawnable = match r.take(1)?[0] {
            0 => false,
            1 => true,
            _ => return Err(invalid()),
        };
    ...
    }
    {
        ...
        if count as u64 > cost.building_prisms || count > (bytes.len() - r.offset) / 95 {
            return Err(invalid());
        }
        ...
        crate::cancellation::checkpoint()?;
            ...
            let mut values = [0; 10];
            for v in &mut values {
                *v = i64::from_le_bytes(r.take(8)?.try_into().unwrap());
            }
            let p = BuildingPrism {
                object_id,
                material,
                ...
                bottom_cm: values[6],
                top_cm: [values[7], values[8], values[9]],
            };
            if !p.valid() {
                return Err(invalid());
            }
            chunk.building_prisms.push(p);
        }
    }
    {
        let count = r.u32()? as usize;
        if count as u64 > cost.asset_convexes || count > (bytes.len() - r.offset) / 121 {
            return Err(invalid());
        }
        for _ in 0..count {
        crate::cancellation::checkpoint()?;
            ...
            if !(4..=32).contains(&vertices)
                || !(4..=60).contains(&faces)
                || vertices * 24 + faces * 3 > bytes.len() - r.offset
            {
                return Err(invalid());
            }
...
mod tests {
    ...
    fn archive_roundtrip_corruption_and_allocation_limits() {
        ...
        for n in 0..bytes.len() {
            assert!(decode_archive(&bytes[..n], &key, chunk.cell, &cost).is_err());
        }
        for i in 0..bytes.len() {
            let mut bad = bytes.clone();
            bad[i] ^= 255;
            assert!(
                decode_archive(&bad, &key, chunk.cell, &cost).is_err(),
                "byte {i}"
            );
        }
        ...
        let mut tiny_cost = cost;
        tiny_cost.triangles = 0;
