...
fn courses_and_evidence_roundtrip_without_changing_driving_content() {
    ...
    for i in 0..2 {
        let body = CourseBody {
            ...
            display_name: format!("Course {i}"),
            world_content_hash: world.clone(),
            mode: Mode::Circuit,
            start_mode: StartMode::Air,
            start_direction: [1, 0],
            checkpoints: vec![
                Checkpoint {
                    position_cm: [1000, 5000, 1000],
                    radius_cm: 1200,
                ...
                Checkpoint {
                    position_cm: [2000, 6000, 2000],
                    radius_cm: 400,
                    shape: CheckpointShape::Hemisphere,
                    placement_mode: PlacementMode::Free,
                    surface_id: String::new(),
                },
            ],
        ...
        if i == 1 {
            ...
            let reference = ValidationReference {
                path: format!("course-validation/{digest}.mevalidation"),
                sha256: digest,
                bytes: bytes.len() as u32,
                world_content_hash: world.clone(),
                geometry_hash: course.definition.geometry_hash().unwrap(),
            };
    ...
    assert_ne!(
        loaded.inspection.package_sha256,
        empty.inspection.package_sha256
    );
    ...
    let reference = d
        .courses
        .iter()
        .find_map(|c| c.validation.as_ref())
        .unwrap();
    assert_eq!(
        reader
            .course_validation(&reference.sha256, &ReadEpoch::default().begin())
            .unwrap(),
        files[&reference.path]
    );
    assert!(reader
        .course_validation(&"f".repeat(64), &ReadEpoch::default().begin())
        .is_err());
    d.terrain_base_cm += 100;
    assert_ne!(
        read_bytes(&pack_bytes(d.clone(), files.clone()).unwrap())
            .unwrap()
            .inspection
            .world_content_hash,
        world
