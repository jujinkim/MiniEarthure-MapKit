...
import struct
import zlib
from urban_probe import PROBE as URBAN_PROBE, fixture as urban_fixture
...
func _initialize() -> void:
    ...
    check(window.data.world_scale == 1.0, "public scale contract")
    check(window.data.recipe_version == 1, "window reports current v1 recipe")
    check(window.data.generated_format_version == 1, "public generated contract")
...
func run() -> void:
    ...
    for i in range(RENDERER.TRIANGLES_PER_BATCH * 2 + 1):
        triangles.append({"object_id": "terrain", "spawnable": true, "surface": "grass", "vertices": [[0, 0, 0], [100, 0, 0], [0, 0, 100]]})
...
def run_engine(command, timeout):
    try:
        ...
        print((error.stdout or b'').decode(errors='replace'), flush=True)
        raise
    output = result.stdout.decode(errors='replace')
    print(output)
    if result.returncode or 'SCRIPT ERROR:' in output or '\nERROR:' in output:
        raise RuntimeError('Godot returned an error diagnostic or nonzero status')
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='mapkit-layout-') as directory:
        ...
        (project / 'urban_probe.gd').write_text(URBAN_PROBE)
        (project / 'urban.json').write_text(json.dumps(urban_fixture(ROOT)))
        subprocess.run([sys.executable, str(ROOT / 'examples/third_party.py'), str(project / 'urban.memap'), str(project / 'urban.json')], check=True, timeout=30)
        ...
        for probe in args.probe or scripts:
            ...
            run_engine([args.godot, '--headless', '--path', str(project), '--script', 'res://' + scripts[probe]], timeout=30)
